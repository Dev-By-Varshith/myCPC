...
export interface CodeSnapshot {
  ...
  codeLength: number;
  lineCount: number;
  code: string; // Full snapshot
}

export interface HesitationPause {
  startTimestamp: number;
  durationSec: number;
  codeLengthAtPause: number;
}

export interface SessionEvent {
  timestamp: number;
  event: 'edit' | 'test_run' | 'test_result' | 'snapshot' | 'session_start';
  code_length?: number;
  diff_summary?: any[];
  verdict?: string;          // for test_result events
  problem_name?: string;     // for session_start
}
...
export interface SessionSummary {
  ...
  endedAt: number;
  totalTimeSec: number;
  hesitationPauses: HesitationPause[];
  rewriteCount: number;
...
export class SessionAnalyzer {
  ...
  static buildSummary(
    ...
    const hesitationPauses: HesitationPause[] = [];
   ...
   */
  static buildReadableTimeline(summary: SessionSummary): string {
    const lines: string[] = [];
  ...
  static classifyStyle(summary: SessionSummary): string[] {
    ...
    if (tleCount >= 2) signals.push('brute-forcer');
    if (waCount >= 4) signals.push('panic-submitter');
    if (rewriteCount >= 3) signals.push('refactorer');
    if (hesitationPauses.length >= 2 && waCount <= 1) signals.push('methodical-planner');
    if (rewriteCount <= 1 && compilationAttempts <= 2) signals.push('incremental-builder');
    if (totalMins < 10 && waCount <= 1 && compilationAttempts <= 2) signals.push('fast-solver');
    if (hesitationPauses.length === 0 && compilationAttempts >= 5) signals.push('trial-and-error');
    if (hesitationPauses.length >= 1 && hesitationPauses[0].durationSec > 300) signals.push('deep-thinker');

    if (signals.length === 0) signals.push('balanced-solver');
    return signals;
  }

  /** Show a VS Code info notification about the session before sending to LLM */
  static getSessionPreview(summary: SessionSummary): string {
   ...
   */
  static analyzePartial(events: SessionEvent[]): {
    ...
    for (let i = 1; i < edits.length; i++) {
      if ((edits[i].timestamp - edits[i - 1].timestamp) / 1000 > 120) hesitationCount++;
    }
    ...
    let rewriteCount = 0;
    for (let i = 1; i < codeLengths.length; i++) {
      if (codeLengths[i - 1] > 100 && codeLengths[i] < codeLengths[i - 1] * 0.8) rewriteCount++;
    }
    ...
    if (waCount >= 4)                             signals.push('panic-submitter');
    if (rewriteCount >= 3)                        signals.push('refactorer');
    if (hesitationCount >= 2 && waCount <= 1)     signals.push('methodical-planner');
    if (rewriteCount <= 1 && compCount <= 2)      signals.push('incremental-builder');
    if (hesitationCount >= 1 && tleCount === 0)   signals.push('deep-thinker');
    if (hesitationCount === 0 && compCount >= 5)  signals.push('trial-and-error');
