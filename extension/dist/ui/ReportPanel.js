...
class ReportPanel {
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel('mycpcDNAReport', '🧬 myCPC DNA Report', column, {
            enableScripts: true,
...
<body>
  ...
  <div class="tip">
    ...
    _getReportHtml(result, summary) {
        ...
        const styleTagsHtml = (styleSignals || []).map(s => `<span class="style-tag">${this._esc(s)}</span>`).join('');
        const struggleHtml = (report?.strugglePoints || []).map((sp, i) => `
    ...
    `).join('');
        const growthHtml = (report?.growthPlan || []).map((g, i) => `
    ...
    `).join('');
        const BACKEND_URL = vscode.workspace.getConfiguration('mycpc').get('backendUrl') || 'http://localhost:3002';
